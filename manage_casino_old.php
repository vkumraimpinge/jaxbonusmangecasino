...
<div class="metabox-holder" id="post-body">
	...
	<div id="leftsidebar_content">
			...
			<input type="text" autocomplete="off" id="title" required="true" value="" size="30" name="post_title">
		...
		<div id="rightsidebar_content">
				...
				<label class="casinoerror2"></label>
							...
							<div id="no_deposit_codes">
							   <p> <img src="'.CASINO_PLUGIN_IMAGEURL.'/image/plus.png" class="casino_addinput" ></p>
						...
						$datamsg =  " New Casino Information Added successfully ";
							   ...
							   if($mode=='update')
							   {
							      ...
							      $wpdb->query($qry) ;
										...
										{ 
										  if(!in_array($ngm,$oldgames)) 
										  {
											...
											if(count($nodeposit)>0)
											{
												 ...
												 $qry_sel = $wpdb->get_results("select * from ".$table_codecasino." where casino_id='".$link."' and type='nodepst'") ; 
												 
												 foreach($qry_sel as $qsl)
												 {
													 $oldvalues[] =   $qsl->id ;
												 }
												 $countold =  count($oldvalues) ;
												
												  if( $countold >0  && $countold < count($nodeposit)  )
												  {
													$qry_upd = "update ".$table_codecasino." set code='".$nodeposit[0]."' where casino_id=".$link." and type='nodepst'" ;
													$wpdb->query($qry_upd)  ; 
													
													$qrynd  = "INSERT INTO ".$table_codecasino."(casino_id,type,code) VALUES('".$link."','nodepst','".$nodeposit[1]."')"; 
												        $wpdb->query($qrynd)  ; 
												  }
												  ...
												  {
														$i = 0 ;
														foreach($nodeposit as $npd)
														{
														$qry_upd = "update ".$table_codecasino." set code='".$npd."' where casino_id=".$link." and type='nodepst' and id=".$oldvalues[$i] ;
														$wpdb->query($qry_upd)  ; 
														$i = $i+1 ;
														}
												  }
										          ...
										          if( $countold ==0  &&  $countold < count($nodeposit)  )
												  {
														foreach($nodeposit as $npd)
														{
														$qrynd  = "INSERT INTO ".$table_codecasino."(casino_id,type,code) VALUES('".$link."','nodepst','".$npd."')"; 
														$wpdb->query($qry_upd)  ; 
														}
												  }
 											}
							       ...
							       $qry_sel_pre = $wpdb->get_results("select id,code from ".$table_codecasino." where type='prev' and casino_id='".$link."'") ;
							       
							       $oldid = array() ; 
							       
							       foreach($qry_sel_pre as $pre)
							       {
									  $oldid[] = $pre->id  ;
								   }
							       ...
							       if(count($oldid) < count($preworking))
							       {
									   foreach($oldid as $old)
									   {
										 $qry_preupd  =  "update " .$table_codecasino." set code='".$preworking[$i]."' where id=".$oldid[$i] ;  
										 $wpdb->query($qry_preupd) ;  
									     $i= $i+1 ;
									   }
									   
									   //echo $i."<br/> " ;
									   //echo count($preworking) ;
									   //echo "<br/> " ;
									   if( count($preworking) > $i )
									   {
											while($i < count($preworking))
											{
											$qrypv  = "INSERT INTO ".$table_codecasino."(casino_id,type,code) VALUES('".$link."','prev','".$preworking[$i]."')"; 
											//echo $qrypv ;
											// $wpdb->query($qrypv)  ; 
											$i++ ;
											//echo $i ;
									   ...
									   if($daily_promo_codes!="")
									   {
										 $qry_sel_pre = $wpdb->get_var("select count(*) from ".$table_codecasino." where type='daily' and casino_id='".$link."'") ; 
										  
										  if($qry_sel_pre >0 || $qry_sel_pre!= "" )
										  {
											$qrydaily  = "INSERT INTO ".$table_codecasino."(casino_id,type,code) VALUES('".$link."','daily','".$daily_promo_codes."')"; 
											$wpdb->query($qrydaily)  ; 
										  }
										  
									   }
								   }
							       $datamsg =  " New Casino Added successfully ";
							   } 
...
function edit_record($id) 
{
 //die('Testing ...');
	...
	$chk_qry3 = $wpdb->get_results("select * from ".$table_codecasino." where casino_id=".$id) ;
